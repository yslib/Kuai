...
local kuai_repl = require("toggleterm.terminal").Terminal:new({
    cmd = "cargo run --bin kuai -- -l",
    direction = "vertical",
    close_on_exit = true,
    hidden = true,
...
local function send_to_dsl_repl()
    ...
    vim.notify("Sending to Kuai REPL:\n" .. text, vim.log.levels.INFO, { title = "Kuai Dev" })
...
vim.keymap.set({ 'n', 'v' }, "<C-e>", function()
    send_to_dsl_repl()
end, { desc = "Send selection to Kuai REPL" })
--
-- vim.keymap.set("n", "<leader>re", function()
-- end, { desc = "Send current line to Kuai REPL" })
