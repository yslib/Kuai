...
fn native_object_trait_accepts_tensor_and_device_dependent_array() -> Result<()> {
    fn check_kind<T: NativeObject + ?Sized>(object: &T, expected: i32) {
        ...
        assert_eq!(
            unsafe { kuai_sys::ku_object_get_value_kind(object.as_raw(), &mut kind) },
