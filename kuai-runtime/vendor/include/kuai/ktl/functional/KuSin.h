...
KU_DEVICE_HOST constexpr auto sin(const T &a) noexcept -> ku_floating_result_type_t<T> {
...
struct ku_sin {
    ...
    KU_DEVICE_HOST constexpr auto operator()(const T1 &t1) const
        noexcept(noexcept(functional_detail::sin(t1))) -> decltype(functional_detail::sin(t1)) {
        return functional_detail::sin(t1);
    }
};
} // namespace kuai
