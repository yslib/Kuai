...
KU_DEVICE_HOST constexpr auto add(const T1 &t1, const T2 &t2) noexcept
...
struct ku_add {
    ...
    KU_DEVICE_HOST constexpr auto operator()(const T1 &t1, const T2 &t2) const
        noexcept(noexcept(functional_detail::add(t1, t2)))
            -> decltype(functional_detail::add(t1, t2)) {
        return functional_detail::add(t1, t2);
    }
};
} // namespace kuai
