...
KU_DEVICE_HOST constexpr auto mul(const T1 &t1, const T2 &t2) noexcept
...
struct ku_mul {
    ...
    KU_DEVICE_HOST constexpr auto operator()(const T1 &t1, const T2 &t2) const
        noexcept(noexcept(functional_detail::mul(t1, t2)))
            -> decltype(functional_detail::mul(t1, t2)) {
        return functional_detail::mul(t1, t2);
    }
};
} // namespace kuai
