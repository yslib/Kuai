...
KU_DEVICE_HOST constexpr KuBool8 ge(const T1 &t1, const T2 &t2) noexcept {
    return logical_not(lt(t1, t2));
}
...
struct ku_ge {
    ...
    KU_DEVICE_HOST constexpr auto operator()(const T1 &t1, const T2 &t2) const
        noexcept(noexcept(functional_detail::ge(t1, t2)))
            -> decltype(functional_detail::ge(t1, t2)) {
        return functional_detail::ge(t1, t2);
    }
};
} // namespace kuai
