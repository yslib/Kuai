...
/kuai (Workspace Root)
├── kuai-cli (二进制: REPL, Daemon, CLI)
│
├── crates/
│   ├── kuai-base      (基础: Span, Diagnostic, Id/Spur)
│   ├── kuai-syntax    (语法: AST, Lexer, Parser, 嵌套 Attribute 定义)
│   ├── kuai-sema      (语义: SymbolTable, Scope, Resolver, Inference)
│   ├── kuai-driver    (核心驱动: AttrRegistry, Pipeline, Session, Context)
│   └── kuai-plugins   (属性插件: @kernel, @export, @einsum 等)
│
└── dev_tools/ (Editor support)


1. KuaiSession (全局静态配置)
在进程启动时创建，持有所有只读、全局的信息。
配置：目标架构（CPU/GPU）、优化等级。
注册表：AttrRegistry（加载了哪些属性插件）。
管理：在 LSP 模式下管理多个 Context。
2. KuaiContext (内存/记忆体)
这是你所谓的“黑板”所在地。它持有所有可变的编译状态。
String Pool：ThreadedRodeo。
Global Scope：Arc<RwLock<Scope>>（存储 dim、func 声明）。
Artifacts：属性生成的后端代码。
3. KuaiCompiler (无状态驱动)
...
逻辑：
当用户打开 a.ku 和 b.ku 时，LSP 服务器持有 Map<Path, Context>。
错误恢复：即便 a.ku 有语法错，LSP 会通过 Parser 的诊断信息并在 Context 中保留“部分正确的 AST”，以支持语法高亮和成员补全。
