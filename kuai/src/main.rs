...
#[command(
    name = "Kuai",
    version = "0.1.0",
    about = "Kuai Programming Language CLI"
)]
struct Args {
    #[arg(short = 'e', long)]
...
fn compile_and_run(input: &str, ctx: &mut Context, compiler: &Compiler, session: &Session) {
    ...
    if result.is_ok() {
        ...
        println!(
            "⚠️  Compilation completed with {} error(s).",
            result.errors().len()
        );
    } else {
        println!(
...
fn start_tcp_server(
    ...
    println!("Kuai REPL Listening on {}", addr);
...
fn run_repl(
    ...
    let mut rl = DefaultEditor::with_config(config)
        .map_err(|e| miette::miette!("Failed to initialize Kuai REPL: {}", e))?;

    // rl.set_helper(Some(KuaiHelper::default()));
