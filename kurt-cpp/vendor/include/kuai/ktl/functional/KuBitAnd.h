...
KU_DEVICE_HOST constexpr auto bit_and(const T1 &t1, const T2 &t2) noexcept
...
struct ku_bit_and {
    ...
    KU_DEVICE_HOST constexpr auto operator()(const T1 &t1, const T2 &t2) const
        noexcept(noexcept(functional_detail::bit_and(t1, t2)))
            -> decltype(functional_detail::bit_and(t1, t2)) {
        return functional_detail::bit_and(t1, t2);
    }
};
} // namespace kuai
