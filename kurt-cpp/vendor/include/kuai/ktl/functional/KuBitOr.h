...
KU_DEVICE_HOST constexpr auto bit_or(const T1 &t1, const T2 &t2) noexcept
...
struct ku_bit_or {
    ...
    KU_DEVICE_HOST constexpr auto operator()(const T1 &t1, const T2 &t2) const
        noexcept(noexcept(functional_detail::bit_or(t1, t2)))
            -> decltype(functional_detail::bit_or(t1, t2)) {
        return functional_detail::bit_or(t1, t2);
    }
};
} // namespace kuai
