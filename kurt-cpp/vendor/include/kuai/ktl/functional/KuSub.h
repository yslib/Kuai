...
KU_DEVICE_HOST constexpr auto sub(const T1 &t1, const T2 &t2) noexcept
    -> ku_binary_result_type_t<T1, T2> {
    using R = ku_binary_result_type_t<T1, T2>;
    if constexpr (ku_has_void_type_v<T1, T2>) {
        ...
        return ku_value_traits<decltype(t1)>::repr(t1) - ku_value_traits<decltype(t2)>::repr(t2);
    }
}
...
struct ku_sub {
    ...
    KU_DEVICE_HOST constexpr auto operator()(const T1 &t1, const T2 &t2) const
        noexcept(noexcept(functional_detail::sub(t1, t2)))
            -> decltype(functional_detail::sub(t1, t2)) {
        return functional_detail::sub(t1, t2);
    }
};
} // namespace kuai
