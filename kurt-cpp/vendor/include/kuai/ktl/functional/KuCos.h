...
KU_DEVICE_HOST constexpr auto cos(const T &a) noexcept -> ku_floating_result_type_t<T> {
...
struct ku_cos {
    ...
    KU_DEVICE_HOST constexpr auto operator()(const T1 &t1) const
        noexcept(noexcept(functional_detail::cos(t1))) -> decltype(functional_detail::cos(t1)) {
        return functional_detail::cos(t1);
    }
};
} // namespace kuai
