...
KU_DEVICE_HOST constexpr auto cosh(const T &a) noexcept -> ku_floating_result_type_t<T> {
    if constexpr (std::is_same_v<T, KuVoid8>) {
        ...
        if constexpr (std::is_same_v<T, KuF32>) {
            ...
            return ::coshf(a); // KuF32
        } else {
            KuF64 val = cast<KuF64>(a);
            if (ku_value_traits<decltype(a)>::isNull(a))
                return ku_value_traits<KuF64>::null();
            return ::cosh(val); // KuF64
        }
    }
}
...
struct ku_cosh {
    ...
    KU_DEVICE_HOST constexpr auto operator()(const T1 &t1) const
        noexcept(noexcept(functional_detail::cosh(t1))) -> decltype(functional_detail::cosh(t1)) {
        return functional_detail::cosh(t1);
    }
};
