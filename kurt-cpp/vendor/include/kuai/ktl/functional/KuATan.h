...
KU_DEVICE_HOST constexpr auto atan(const T &a) noexcept -> ku_floating_result_type_t<T> {
...
struct ku_atan {
    ...
    KU_DEVICE_HOST constexpr auto operator()(const T1 &t1) const
        noexcept(noexcept(functional_detail::atan(t1))) -> decltype(functional_detail::atan(t1)) {
        return functional_detail::atan(t1);
    }
};
} // namespace kuai
