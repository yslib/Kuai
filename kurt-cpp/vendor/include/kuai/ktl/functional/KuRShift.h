...
KU_DEVICE_HOST constexpr auto rshift(const T1 &t1, const T2 &t2) noexcept -> decltype(t1 >> t2) {
...
struct ku_rshift {
    ...
    KU_DEVICE_HOST constexpr auto operator()(const T1 &t1, const T2 &t2) const
        noexcept(noexcept(functional_detail::rshift(t1, t2)))
            -> decltype(functional_detail::rshift(t1, t2)) {
        return functional_detail::rshift(t1, t2);
    }
};
} // namespace kuai
