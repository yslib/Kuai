...
struct ku_exp {
    ...
    KU_DEVICE_HOST constexpr auto operator()(const T &a) const
        noexcept(noexcept(functional_detail::exp(a))) -> decltype(functional_detail::exp(a)) {
        return functional_detail::exp(a);
    }
};
} // namespace kuai
