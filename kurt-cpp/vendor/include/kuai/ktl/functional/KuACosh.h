...
KU_DEVICE_HOST constexpr auto acosh(const T &a) noexcept -> ku_floating_result_type_t<T> {
    if constexpr (std::is_same_v<T, KuVoid8>) {
        return ku_value_traits<KuF64>::null();
    } else {
        if constexpr (std::is_same_v<T, KuF32>) {
            if (a < 1 || ku_value_traits<decltype(a)>::isNull(a))
                return ku_value_traits<KuF32>::null();
            return ::acoshf(a); // KuF32
        } else {
            auto val = cast<KuF64>(a);
            if (val < 1 || ku_value_traits<decltype(val)>::isNull(val))
                return ku_value_traits<KuF64>::null();
            return ::acosh(val); // KuF64
        }
    }
}
...
struct ku_acosh {
    ...
    KU_DEVICE_HOST constexpr auto operator()(const T1 &t1) const
        noexcept(noexcept(functional_detail::acosh(t1))) -> decltype(functional_detail::acosh(t1)) {
        return functional_detail::acosh(t1);
    }
};
} // namespace kuai
