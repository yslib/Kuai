...
KU_DEVICE_HOST constexpr KuBool8 le(const T1 &t1, const T2 &t2) noexcept {
...
struct ku_le {
    ...
    KU_DEVICE_HOST constexpr auto operator()(const T1 &t1, const T2 &t2) const
        noexcept(noexcept(functional_detail::le(t1, t2)))
            -> decltype(functional_detail::le(t1, t2)) {
        return functional_detail::le(t1, t2);
    }
};
