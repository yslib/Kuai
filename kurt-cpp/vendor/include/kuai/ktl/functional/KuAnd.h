...
KU_DEVICE_HOST constexpr KuBool8 logical_and(const T1 &t1, const T2 &t2) noexcept {
    if constexpr (ku_has_void_type_v<T1, T2>) {
        ...
        return ku_value_traits<KuBool8>::fromBool(
            ku_value_traits<KuBool8>::repr(cast<KuBool8>(t1)) != KU_BOOL_FALSE
            && ku_value_traits<KuBool8>::repr(cast<KuBool8>(t2)) != KU_BOOL_FALSE);
    }
}
...
struct ku_and {
    ...
    KU_DEVICE_HOST constexpr auto operator()(const T1 &t1, const T2 &t2) const
        noexcept(noexcept(functional_detail::logical_and(t1, t2)))
            -> decltype(functional_detail::logical_and(t1, t2)) {
        return functional_detail::logical_and(t1, t2);
    }
};
