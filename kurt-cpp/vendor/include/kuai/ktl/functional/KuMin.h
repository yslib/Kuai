...
KU_DEVICE_HOST constexpr auto min(const T1 &t1, const T2 &t2) noexcept
    -> ku_promotion_type_t<T1, T2> {
    using R = ku_promotion_type_t<T1, T2>;
    return ku_value_traits<KuBool8>::repr(lt(t1, t2)) == KU_BOOL_TRUE ? cast<R>(t1) : cast<R>(t2);
}
...
struct ku_min {
    ...
    KU_DEVICE_HOST constexpr auto operator()(const T1 &t1, const T2 &t2) const
        noexcept(noexcept(functional_detail::min(t1, t2)))
            -> decltype(functional_detail::min(t1, t2)) {
        return functional_detail::min(t1, t2);
    }
};
